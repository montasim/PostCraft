...
async function testZhipuAnthropic() {
  ...
  if (!key) {
    console.log("❌ - zhipu (Anthropic) - glm-5.1 - https://api.z.ai/api/anthropic/v1/messages (No Key)");
    return;
  }
  ...
  try {
    ...
    if (res.ok) {
      console.log(`✅ - zhipu (Anthropic) - glm-5.1 - https://api.z.ai/api/anthropic/v1/messages`);
    } else {
      ...
      console.log(`❌ - zhipu (Anthropic) - glm-5.1 - https://api.z.ai/api/anthropic/v1/messages (${msg})`);
    }
  } catch(e: any) {
    console.log(`❌ - zhipu (Anthropic) - glm-5.1 - https://api.z.ai/api/anthropic/v1/messages (${e.message})`);
  }
}
...
async function testStandardProviders() {
  ...
  for (const t of tests) {
    ...
    try {
      ...
      const caller = PROVIDER_CALLERS[t.provider as keyof typeof PROVIDER_CALLERS];
