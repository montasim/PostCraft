...
import { TwitterPostItem } from "./twitter-post-item"

export default async function TwitterInsightsPage() {
  ...
  const posts = await TwitterPost.find({ userId: session.user.id })
    .sort({ createdAt: -1 })
    .lean()
  ...
  if (account && account.accessToken) {
    try {
      ...
      if (profileRes.ok) {
        ...
        if (profile.data) {
          twitterUser.name = profile.data.name || twitterUser.name
          twitterUser.image = profile.data.profile_image_url || twitterUser.image
        }
      }
    } catch (e) { }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          <h1 className="text-md font-semibold">Twitter Posts</h1>
          <p className="text-sm text-muted-foreground">
            View your published and scheduled Twitter posts.
          </p>
        </div>
      ...
      {serializedPosts.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-4 items-start">
          {serializedPosts.map((post: any) => (
            <TwitterPostItem key={post._id} post={post} user={twitterUser} />
          ))}
        </div>
      )}
    </div>
