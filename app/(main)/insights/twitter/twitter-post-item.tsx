      ...
      <div className="w-full h-full rounded-lg border bg-card text-card-foreground p-0 shadow-sm relative transition-opacity duration-200 flex flex-col" style={{ opacity: isDeleting ? 0.5 : 1 }}>
        <div className="flex items-start gap-3 border-b px-4 py-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-muted-foreground truncate">@{user?.name?.toLowerCase().replace(/\s+/g, '') || "user"}</p>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="p-0 sm:max-w-[550px] gap-0 overflow-hidden bg-background">
          <DialogHeader className="border-b px-6 py-4">
            <DialogTitle className="text-xl font-normal text-foreground">Edit tweet</DialogTitle>
          </DialogHeader>
                      ...
                      <TooltipProvider delayDuration={200}>
                        <Tooltip>
                          ...
                          <TooltipContent className="max-w-[260px] text-center p-3 leading-relaxed" side="top" sideOffset={8}>
                            <p>We do not schedule posts directly on Twitter. Instead, PostCraft holds your post securely on our servers and automatically publishes it to your account at the scheduled time.</p>
                          </TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
                    </div>
