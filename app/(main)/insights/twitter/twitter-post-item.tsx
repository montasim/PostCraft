...
import { deleteTwitterPost, updateTwitterPost } from "./actions"
...
export function TwitterPostItem({ post, user }: { post: any; user: any }) {
  const isPublished = post.status === "published"
  const isScheduled = post.status === "scheduled"
  const isFailed = post.status === "failed"

  let postLink = "#"
  if (isPublished && post.tweetId) {
    postLink = `https://twitter.com/i/web/status/${post.tweetId}`
  }
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true)
    try {
      await deleteTwitterPost(post._id.toString())
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-around border-t px-2 py-1">
          {[
            { icon: IconMessageCircle2, label: "Reply" },
            { icon: IconRepeat, label: "Repost" },
            { icon: IconHeart, label: "Like" },
            { icon: IconExternalLink, label: "Share" },
          ].map(({ icon: Icon, label }) => (
            <button
              key={label}
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="gap-0 overflow-hidden bg-background p-0 sm:max-w-[550px]">
          ...
          <div className="flex max-h-[70vh] flex-col overflow-y-auto">
            <div className="flex items-center gap-3 px-6 py-4">
              ...
              <div>
                ...
                <button className="mt-1 flex items-center gap-1 rounded-full border border-muted-foreground/30 px-3 py-1 text-xs font-semibold text-muted-foreground transition-colors hover:bg-muted hover:text-foreground">
              ...
              <div className="mt-5 space-y-5">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className="h-11 rounded-lg border-0 bg-muted/40 px-4 text-sm font-semibold text-primary shadow-none focus-visible:ring-1 focus-visible:ring-primary/50"
                  />
                </div>
                ...
                {isScheduled && (
                  <div className="space-y-2 pt-1">
                    <div className="ml-1 flex items-center gap-1.5">
                      ...
                      <TooltipProvider delayDuration={200}>
                        <Tooltip>
                          ...
                          >
                            <p>
                              We do not schedule posts directly on Twitter.
                    ...
                    <Input
                      ...
                      className="h-11 rounded-lg border-0 bg-muted/40 px-4 text-sm shadow-none focus-visible:ring-1 focus-visible:ring-primary/50 dark:[color-scheme:dark]"
          ...
          <DialogFooter className="m-0 flex items-center justify-end rounded-none border-t bg-background px-6 py-4 sm:justify-end">
            <Button
              className="h-10 rounded-full bg-primary px-6 font-semibold text-primary-foreground transition-colors hover:bg-primary/90"
              onClick={handleEdit}
              disabled={isSaving}
            >
