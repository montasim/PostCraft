          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[11px] text-muted-foreground pt-0.5">
              {isScheduled && post.scheduledTime
                ...
                : `${formatDistanceToNow(new Date(post.createdAt), { addSuffix: true })}`} · <IconWorld className="inline-block h-3 w-3 -mt-0.5" />
            </p>
          </div>
          <DropdownMenu>
          ...
          {post.hashtags && post.hashtags.length > 0 && (
            <div className="flex flex-wrap gap-1.5 pt-1">
              {post.hashtags.map((tag: string) => (
                <span key={tag} className="text-sm font-semibold text-[#1877F2] hover:underline cursor-pointer">
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="p-0 sm:max-w-[550px] gap-0 overflow-hidden bg-background">
          ...
          <div className="flex flex-col overflow-y-auto max-h-[70vh]">
            <div className="px-6 py-4 flex items-center gap-3">
              ...
              <div>
                ...
                <button className="mt-1 flex items-center gap-1 rounded-sm bg-muted/50 px-2 py-0.5 text-[11px] font-semibold text-muted-foreground hover:bg-muted transition-colors">
                  <IconWorld className="h-3 w-3" />
                  Public
                  <IconChevronDown className="h-3 w-3 ml-0.5" />
                </button>
              </div>
            </div>
            ...
            <div className="px-6 pb-6">
              <Textarea 
                ...
                className="min-h-[200px] border-0 focus-visible:ring-1 focus-visible:ring-[#1877F2]/50 px-4 py-3 resize-none text-[15px] leading-relaxed text-foreground shadow-none placeholder:text-muted-foreground bg-muted/40 rounded-lg"
              ...
              <div className="mt-5 space-y-5">
                <div className="space-y-2">
                  ...
                  <Input 
                    ...
                    className="h-11 text-sm font-semibold text-[#1877F2] border-0 focus-visible:ring-1 focus-visible:ring-[#1877F2]/50 rounded-lg px-4 shadow-none bg-muted/40"
                  />
                </div>
                ...
                {isScheduled && (
                  <div className="space-y-2 pt-1">
                      ...
                      <TooltipProvider delayDuration={200}>
                        <Tooltip>
                          ...
                          <TooltipContent className="max-w-[260px] text-center p-3 leading-relaxed" side="top" sideOffset={8}>
                            <p>We do not schedule posts directly on Facebook. Instead, PostCraft holds your post securely on our servers and automatically publishes it to your account at the scheduled time.</p>
                          </TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
                    </div>
                    <Input 
                      ...
                      className="h-11 text-sm border-0 focus-visible:ring-1 focus-visible:ring-[#1877F2]/50 rounded-lg px-4 shadow-none bg-muted/40 dark:[color-scheme:dark]"
          ...
          <DialogFooter className="m-0 px-6 py-4 border-t flex items-center justify-end sm:justify-end bg-background rounded-none">
            <Button 
              className="rounded-full px-6 h-10 font-semibold bg-[#1877F2] hover:bg-[#166fe5] text-white transition-colors" 
              onClick={handleEdit} 
              disabled={isSaving}
            >
