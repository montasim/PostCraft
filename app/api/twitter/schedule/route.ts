...
export async function POST(req: Request) {
  try {
    ...
    logger.info(
      ...
      "Successfully scheduled Twitter post event sent to queue"
    )

    return NextResponse.json({
      success: true,
      message: "Successfully scheduled Twitter post",
      id: dbPost._id,
    })
  } catch (error) {
    logger.error(
      { err: error instanceof Error ? error.message : String(error) },
      "Twitter schedule error"
    )
    return NextResponse.json(
      { error: "Failed to schedule Twitter post" },
      { status: 500 }
    )
  }
}
