...
export async function createRun(data: Partial<ITrendingRunDoc>): Promise<ITrendingRunDoc> {
  await connectDB()
  return TrendingRun.create(data)
}

export async function findRunsByWorkspace(workspaceId: string, limit = 50): Promise<ITrendingRunDoc[]> {
  await connectDB()
  return TrendingRun.find({ workspaceId }).sort({ ranAt: -1 }).limit(limit).lean()
}
...
export async function updateRunStatus(
  ...
  await connectDB()
  await TrendingRun.updateOne({ _id: runId }, { $set: { status, error: error ?? null } })
}

export async function updateRunSourceItems(runId: string, sourceItems: ISourceItemDoc[]): Promise<void> {
  await connectDB()
  await TrendingRun.updateOne({ _id: runId }, { $set: { sourceItems } })
}

export async function updateRunGenerationIds(runId: string, generationIds: string[]): Promise<void> {
  await connectDB()
